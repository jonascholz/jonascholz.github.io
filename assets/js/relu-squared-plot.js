document.addEventListener('DOMContentLoaded', function () {
    const container = document.getElementById('relu-squared-plot');
    ...
    for (let i = 0; i <= numPoints; i++) {
        ...
        const xSquared = x * x;
        const y = Math.max(0, xSquared); // ReLU(x²) = x² since x² ≥ 0
        reluData.push({ x, y });
    }

    // Calculate mean of ReLU(x²) outputs for inputs in [-1, 1]
    // ReLU(x²) = x² for all x, mean of x² over [-1, 1] is 1/3
    const meanValue = 1/3;
    ...
    plotGroup.appendChild(createText(legendX + 40, legendY + legendSpacing, 'Mean ≈ 0.33', {
        anchor: 'start',
        baseline: 'middle',
        fill: meanColor,
        fontSize: '14',
