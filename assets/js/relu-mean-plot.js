document.addEventListener('DOMContentLoaded', function () {
    const container = document.getElementById('relu-mean-plot');
    ...
    // Colors
    const reluColor = '#047857'; // dark emerald
    ...
    for (let i = 0; i <= numPoints; i++) {
        ...
        const y = Math.max(0, x); // ReLU function
        reluData.push({ x, y });
    }

    // Calculate mean of ReLU outputs for inputs in [-1, 1]
    // For uniform distribution in [-1, 1]:
    // - Half the inputs (-1 to 0) output 0
    // - Half the inputs (0 to 1) output x with mean 0.5
    // Overall mean = 0.5 * 0 + 0.5 * 0.5 = 0.25
    const meanValue = 0.25;
    ...
    // plotGroup.appendChild(createText(legendX + 40, legendY + legendSpacing, 'Mean = 0.25', {
    //     anchor: 'start',
    //     baseline: 'middle',
    //     fill: meanColor,
    //     fontSize: '14',
