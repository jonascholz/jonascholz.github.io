...
def lif_scan_wrapper(params: LIFParams, state: LIFState, inputs: jnp.ndarray):
    new_state = lif_step(params, state, inputs)
    return new_state, None  # carry, append_for_history
...
Note that the state history is no longer a list of states, but instead an object where every property is a list. Again, it's a bit weird, but feel free to print some values to get a feel for it.
